  ...
  return (
    <Card className="group hover:shadow-lg transition-all duration-200 border-0 shadow-sm bg-white">
      ...
      <CardContent className="p-4 space-y-3">
        <div>
          <p className="text-sm font-medium text-gray-600 uppercase tracking-wide">{product.itemDetails.brand}</p>
          <h3 className="font-semibold text-gray-900 line-clamp-2 leading-tight">{product.itemDetails.name}</h3>
        </div>

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-gray-900">£{product.price.buy.toFixed(2)}</span>
            {product.price.discount && product.price.discount > 0 && (
