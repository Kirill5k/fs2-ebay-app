  ...
  "An ArgosClient" should {
    ...
    "return relevant deliverable or reservable items" in {
      ...
      val client = MockCurlImpersonateClient.make[IO](
        url1 -> (StatusCode.Ok, FileReader.fromResources("argos/search-success-page-1-response.json")),
        url2 -> (StatusCode.Ok, FileReader.fromResources("argos/search-success-page-2-response.json"))
      )
