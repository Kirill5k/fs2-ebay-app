...
private[mappers] object GameDetailsMapper {
  ...
  private val LEVEL3_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "Formula One World Championship", "\\bNo Figure(s)?\\b", "PREVIOUSLY FIFA",
    ...
    "\\bsave s\\b", "\\bdmc\\b", "\\bBNI[BP]\\b", "\\bNSO\\b", "\\bNM\\b", "\\bLRG(( )?\\d+)?\\b",
