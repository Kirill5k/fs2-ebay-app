  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "upcoming.{1,5}game", "various video games", "\\d x nintendo switch games", "ps games x \\d", "PS4 game \\d+", "\\d+( )?x ps4( video)? games",
        ...
        "(starter|craft|diy|achievement|icon|BADGE PIN)( )?(set|pack|bundle|kit)", "starlink(?s).*pack", "digital edition",
        ...
        "coin", "skins", "(pencil|traveller|system|collectors|empty|steel|slim|display|switch)( )?((game )?box|case)", "soundtrack", "poster", "protection accessor", "PLAYSTAND", "(character|SILICONE) skin",
