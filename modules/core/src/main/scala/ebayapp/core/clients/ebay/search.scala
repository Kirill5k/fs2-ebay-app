  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(case|variety|accessor(ies|y)|storage|charge|robot|dice|charging) (system|set|kit|box)", "no dis(c|k)", "Season( \\d)? Pass", "pass set", "keychain",
        ...
        "official (server|magazin)", "Option File", "offline", "online", "unlock all", "mini dock", "Press Release", "Pre( )?Release",
        "coin", "skins", "(pencil|traveller|system|collectors|empty|steel|slim|display)( )?((game )?box|case)", "soundtrack", "poster", "protection accessor", "PLAYSTAND", "(character|SILICONE) skin",
        ...
        "sniper thumbs", "(game|skin|thumb|Silicone|floating) grip", "(screen|grip|case) (protector|combat|stick)", "(sports|leg) strap", "Cleaning Cloth",
        ...
        "Gigaset COMFORT", "Curtain(s)? for Patio", "\\d+( )?mAh",
        ...
        "figure(s)? bundle", "k eso", "(mini|plush|gift|grip(s)?) (set|toy)", "pad pro", "thumb( )?(tread|stick|pack)", "cable (guy|adapter|pack)", "Philips",
        "pre(\\s+)?(order|sale)", "(steel|art)( )?book", "(game|vinyl|mini|pvc)( )?figure", "collectable", "collectible", "remote control", "(ethernet|patch|aux|charg(ing|e)|power|av|adapter) cable",
        ...
        "(usa|hungarian|scandinavian|asian|korea(n)?|polish|asia(n)?|german|promo(tional)?|starter|demo|french|jap(an)?(ese)?|cz|dutch|italian|spanish|us(a)?|digital|nordic|\\bau\\b|multi(-)?language) (release|cover|pack|box|import|item|disc|vers|copy)",
        ...
        "(farm|season|unique|x10|admission|party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo",
        "diablo.*(farm|season|unique|x10|admission|party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bXP\\b|\\bPC\\b)",
        ...
        "LED Light Box", "check description", "loot run", "bluetooth", "(personalised|gift) mug", "Expansion Pack", "portable stand",
        ...
        "Genshin Impact.*(weapon|primos|reward|code)",
