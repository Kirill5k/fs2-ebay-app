...
private[mappers] object GameDetailsMapper {
  ...
  private val LEVEL2_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "\\d{3,}\\s+\\d{4,}",
    "for playstation( )?vr",
    "((ps( )?)?(vr|move)|kinect|fit|balance board) (needed|required|compatible)",
    ...
    "Now Released(?s).*$",
    "Release date(?s).*$",
    "includes( free(?s).*$|.{0,20}pack(?s).*$| \\w+)",
    "amazon.{0,20}exclusive(?s).*$",
    ...
    "(?<=20\\d\\d).*formula(?s).*",
    "(?<=Fifa 21.*) (NXT LVL|Next level)",
    "(?<=PGA Tour.*) golf.*",
    "(?<=Super meat) (?=boy)",
    "(?<=harry potter).*years.*",
    "(?<=Gran)d(?= turismo)",
    "(?<=No Mans Sky) Beyond",
    "(?<=Grand Theft Auto (\\d|\\b[VI]{1,2}\\b)).*map.*",
    "\\bGT(S|\\d)?\\b .*(?=gran turismo)",
    "(?<=Turismo( (\\d|sport))?) \\bGT(\\d|S)?\\b",
    "(?<=gears) of war(?= 5)",
    "bandicoot( CTR)? (?=team racing)",
    "(?<=resident evil (vii|7)).*biohazard.*",
    "(?<=cyber) (?=punk)",
    "(?<=Warhammer) (40k|40( )?000)",
    "Formula (1|One)\\s+(?=F1)",
    "Marvel(s)?\\s+(?=(iron man|deadpool|Spider))",
    "(?<=Sonic) Hedgehog",
    "Formula (1|One)\\s+(?=F1)",
    "Marvel(s)?\\s+(?=(iron man|deadpool|Spider))",
    "(?<=\\b[ivx]{1,4}\\b)(\\s+)\\d+",
  ...
  private val LEVEL3_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "(s|c)ellophane wrapped",
    "free\\s+upgrade",
    "(official )?Strategy Combat( guide)?",
    "(First Person|FPS|1st) Shooter",
    "(american|soccer) football( 20\\d\\d)?",
    "(racing|auto|golf|football) sport(s)?",
    "Adventure role playing",
    "ice hockey",
    "shoot em up",
    "Sport(s)? (skateboard|basketball|football)",
    "football soccer( sim(ulator)?)?",
    "action stealth",
    "(car|motorcycles|rally) (Driving|Racing)",
    ...
    "\\b(\\d|both)?( )?(dis(c|k)(s)?|cd(s)?)( (version|set|mint))?\\b",
    "\\d \\d players",
    "1 ONE",
    "flash sale",
    "Fun Kids Play( Console)?",
    "with all extras",
    "free gift",
    "deleted title",
    "\\bID\\d+\\w",
    "SEEDESCRIPTIONFORDETAILS",
    "cheapest.*on ebay",
    "strategy guide",
    ...
    "Stric(t|k)ly limited",
    "Ultimate Fighting Champion(ship)?",
    "available now",
    "cross gen",
    "Lenticular (cover|Sleeve)",
    "plays perfect",
    "100 trusted seller",
    "(Backward(s)? )?compatible",
    "(bundle|physical) copy",
    "nuevo",
    "(big|steel)( )?box( version)?",
    "Scratch Free",
    "no scratches",
    "(map\\s+)?(manual(s)?|instructions)(\\s+map)?( (is|are))?( not)?( (included|missing))?",
    "100 ebayer",
    "(condition )?very good",
    "reorderable",
    "(posted|sent|dispatch).{0,10}day( all orders placed)?( by \\d pm)?",
    "(last one )?in stock( now)?",
    "never( been)? (opened|played)",
    "(only )?played (once|twice)",
    "(great|best) price( on ebay)?",
    "Special Reserve",
    "Expertly Refurbished Product",
    "(quality|value) guaranteed",
    "(trusted )?(eBay|best|from ebays biggest) (shop|Seller)(s)?",
    ...
    "deluxe",
    "standard",
    "Official(l)?(y)? Licen(s|c)ed",
    "machine cleaned",
    "\\bctr\\b",
    "\\bgoty\\b",
    "mult(i)?( )?lang(uage)?(s)?( in game)?",
    "(with )?(fast|free|(1|one|same|next)( )?day)( )?(delivery|dispatch|post|\bPO\\b)",
    "for kids",
    "fast free",
    "blu( )?ray( film)?",
    "Console Exclusive",
    ...
    "(\\bcase\\b|box|map).{0,20}(cart(ridge)?|included|complete|manual)",
    "(super|very) rare",
    "award winning",
    "Instruction(s)? Book",
    ...
    "add( |-)?on(( content)? pack)?",
    "jeu console",
    "\\b(Rated )?(For )?age(s)?( )?\\d+(\\s+over)?\\b",
    "must see",
    "see (pictures|photos|pics)",
    "Refurbished",
    "shrink( )?wrapped",
    "\\bcert( )?\\d+\\b",
    "no dlc(s)?( included)?",
    "dlc( not)? included",
    "(still )?in( (plastic|cellophane))? wrap(p)?(ing|er)",
    "\\brated \\d+\\b",
    "\\d supplied",
    ...
    "\\bfr\\b",
    "\\bes\\b",
    "(in )?\\bvg(c| con(d)?(ition)?)?\\b( condition)?",
    "\\ban\\b",
    "\\bLTD\\b",
    "\\b\\w+VG(C)?\\b",
    "\\bns\\b",
    ...
    "\bXBOX\b",
    "\bCASED\b",
    "(100 )?((all|fully) )?complete( (map|mint|instructions|package))?",
    "(condition )?NEW(\\s+)?$"
  ).mkString("(?i)", "|", "")
