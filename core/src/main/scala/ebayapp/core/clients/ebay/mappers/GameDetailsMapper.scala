...
private[mappers] object GameDetailsMapper {
  ...
  private val LEVEL3_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "must see", "see (pictures|photos|pics)", "Refurbished", "shrink( )?wrapped", "\\bcert( )?\\d+\\b", "no dlc(s)?( included)?", "dlc( not)? included", "(still )?in wrap(p)?(ing|er)",
    ...
    "\\bPA2\\b", "\\bWi1\\b", "\\bENG\\b", "\\bVGWO\\b", "\\bFPS\\b", "\\b(PS( )?)?VR\\b( version)?", "\\bDEFY\\b", "\\bArgos\\b", "\\bGD\\b",
