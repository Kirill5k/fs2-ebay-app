  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(canvas|replacement|cover|carry|travel(er)?|commuter|carrying|just( the)?|no|hard|storage|game|vault|phone|card|foreign|metal|console|protection|protective|nintendo switch)\\s+(pouch|case|bag)",
        ...
        "(pvp|pve)(?s).*ark", "ark(?s).*(pvp|pve)", "Code(-| )?in(-| )?(a)?(-| )?Box",
        ...
        "(cod|of duty)(?s).* (points|boost|unlock|warzone)", "Destiny 2", "heatsink",
        "Temperature Sensor", "RGB LED", "powerstand",
