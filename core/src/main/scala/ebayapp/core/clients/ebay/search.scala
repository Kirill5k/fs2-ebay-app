  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(rune|item|gold|softcore|(nm|nightmare) dungeon|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo", "diablo.*(rune|item|gold|softcore|(nm|nightmare) dungeon|\\bGEM(s)?\\b|weapon|\\bXP\\b|\\bPC\\b)",
        "nexigo", "fan cooling", "Eve Valkyrie", "invaders plush", "xbox original", "Original Xbox", "Accessories Only",
        "battery lid", "lag switch", "media remote", "game \\d+ for", "NTSCjp", "ZX SPECTRUM", "all platforms",
