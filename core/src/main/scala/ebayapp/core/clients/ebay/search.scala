  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "arabic", "slovakian", "czech", "NTSC USA", "to be updated", "(ps\\d|xbox( one)?) digital", "Dreamcast", "Handheld System", "PC MAC",
        ...
        "villager(?s).*animal crossing", "animal crossing(?s).* (\\d+M|item|cafe|park|bell|mil|diy|material|turnip|ticket|recipe|fossil|dino|egg|gold)",
        ...
        "(cod|of duty|mw2|warzone)(?s).*(crossbow|code|items|camo|points|boost|hyper|unlock|mountain dew|warzone|skin|level|card|\\bXP\\b)", "Destiny 2", "Destiny.*taken king", "heatsink", "Destiny\\s+the coll",
        "Temperature Sensor", "RGB LED", "powerstand", "not duped", "amiibo", "DIY material", "Steep X ",
        ...
        "minecraft.*(item)", "No Mans Sky.*(Ship Pack|Interceptor)", "BO3.*(Divinium|Cryptokey)",
        "Hogwarts Legacy.*(onyx|potion|shop)", "DYING LIGHT.*(EGG-SPLOSIVE|THROWABLE)",
        ...
        "forza.*Wheelspin", "demon soul.*level", "\\bBDSP\\b", " DS PEGI", "\\bWII\\b", "Roblox",
