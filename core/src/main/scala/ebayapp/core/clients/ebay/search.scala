  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(pvp|pve|reaper|lvl)(?s).*ark", "ark(?s).*(pvp|breeding|deadpool|pve|reaper|lvl|Tek)", "Code(-| )?in(-| )?(a)?(-| )?Box",
        "diablo 3(\\s+(\\w+|\\d+)){6,}", "fortnite", "placeholder \\d", "character skin", "overwatch( -)? (ps|xbox|legendary|origins|game)",
        "(rune|items).*diablo", "diablo.*(rune|items)", "nexigo", "fan cooling", "Eve Valkyrie", "invaders plush",
        ...
        "forza.*Wheelspin", "demon soul.*level", "\\bBDSP\\b",
