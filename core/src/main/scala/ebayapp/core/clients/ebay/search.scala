  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "TOPModel", "Sleeping Mask", "Juwel", "game( +)?watch", "Wonder Pin",
        ...
        "placeholder( listing)? \\d", "^game \\d",
        "(party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo", "Trigger Treadz", "Painting on Canvas",
        "diablo.*(party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bXP\\b|\\bPC\\b)", "Life is Strange Before the Storm",
        ...
        "modern warfare.*(camo)", "YuGiOh.*(Gems)",
