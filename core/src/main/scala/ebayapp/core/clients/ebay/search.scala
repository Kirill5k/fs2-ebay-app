  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo",
        ...
        "Hogwarts Legacy.*(onyx|potion|shop)", "DYING LIGHT.*(EGG-SPLOSIVE|THROWABLE)", "PS Crossplay", "V7332",
        ...
        "PlayStation 3.?PS3", "PlayStation 2.?PS2", "\\bPlayStation (2|3)\\b", "gameboy"
      ).mkString("^.*?(?i)(", "|", ").*$").r
      // format: on
