  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "coin", "skins", "(collectors|empty|steel|slim|display)( )?((game )?box|case)", "soundtrack", "poster", "protection accessor", "PLAYSTAND", "Press Release", "starlink(?s).*pack",
        ...
        "starter (set|pack|bundle|kit)", "figure(s)? bundle", "k eso", "(mini|gift) toy", "pad pro", "cable (guy|adapter|pack)",
        "pre(\\s+)?(order|sale)", "season pass", "(steel|art)( )?book", "(game|mini)( )?figure", "collectable", "collectible", "remote control", "(patch|aux|charg(ing|e)|power|av) cable",
        ...
        "(rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo", "Trigger Treadz",
        "diablo.*(rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bXP\\b|\\bPC\\b)", "Life is Strange Before the Storm",
        "nexigo", "fan cooling", "Eve Valkyrie", "invaders plush", "xbox original", "Original Xbox", "Accessories Only", "figpin",
        "battery lid", "lag switch", "media remote", "game \\d+ for", "NTSCjp", "ZX SPECTRUM", "all platforms", "AutoPop Trophies",
        ...
        "animal crossing", "SCARLET.*VIOLET", "kinect sensor", "gaming locker", "eso gold", "gaming locker",
        ...
        "Temperature Sensor", "RGB LED", "powerstand", "not duped", "amiibo", "DIY material", "Steep X ", "(Switch|lag) pedal", "game not included",
        ...
        "starfield.*(credit|max pow)",
        "YuGiOh.*(Gems)",
        "minecraft.*(item)", "No Mans Sky.*(Ship Pack|Interceptor)", "BO3.*(Divinium|Cryptokey)", "\\bto update\\b",
        "Hogwarts Legacy.*(onyx|potion|shop)", "DYING LIGHT.*(EGG-SPLOSIVE|THROWABLE)", "PS Crossplay", "V\\d{4,5}(A|Z)",
        "(rune|million|level)(?s).*elden ring", "elden ring(?s).*(rune|million|level)", "Pok(e|é)?mon", "games lot",
        ...
        "forza.*Wheelspin", "demon soul.*level", "\\bBDSP\\b", " DS PEGI", "\\bWII\\b", "Roblox", "\\bVPN\\b", "\\bDVD\\b", "\\bNDS\\b",
        ...
        "X(BOX)?(\\s+)?360","(super )?nintendo ((3)?ds|wii|eshop)", "\\b(3)?DS\\b ninten(t|d)o", "gamecube", "\\bN64\\b", "\\bDS\\b game", "nintendo (3)?ds",
