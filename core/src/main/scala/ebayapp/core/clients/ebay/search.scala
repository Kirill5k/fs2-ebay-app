  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "switch.*(alu|unicorn).*\\bcase\\b", "credit(?s).*accoun", "credits",
        "(\\d+|rune|perk|microsoft|skill|(e)?xp(erience)?)\\s+(stats|points)", "(memory|trading|post|sd|nfc( mini)?)( )?card", "stickers",
        "(subscription|gift|ps\\d|steam|network|will send)( |-)?(card|code|key)", "disc \\d missing", "(instant|digital) delivery", "faulty", "damaged",
        ...
        "(case|variety|accessor(ies|y)|storage|charge|robot|dice) (system|set|kit|box)", "no dis(c|k)", "Season( \\d)? Pass",
