  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "\\b200[0-9]\\b",
        "(\\d+|rune|perk|microsoft|skill|(e)?xp(erience)?)\\s+(stats|points)", "(memory|trading|post|sd|nfc( mini)?)( )?card", "stickers", "digital delivery",
        ...
        "(case|variety|accessor(ies|y)|storage|charge|robot|dice) (system|set|kit|box)", "no dis(c|k)", "Season( \\d)? Pass", "switch.*(alu|unicorn).*\\bcase\\b",
        ...
        "fortnite", "character skin", "overwatch\\s+(sony|ps|xbox|legendary|origins|game)",
        "(unique|x10|admission|party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bPC\\b|\\bXP\\b).*diablo",
        "diablo.*(unique|x10|admission|party|rune|item|gold|softcore|(nm|nightmare) dungeon|modded|\\bGEM(s)?\\b|weapon|\\bXP\\b|\\bPC\\b)",
        "nexigo", "fan cooling", "Eve Valkyrie", "invaders plush", "xbox (original|classic)", "Original Xbox", "figpin",
        "battery lid", "lag switch", "media remote", "game \\d+ for", "NTSCjp", "ZX SPECTRUM", "all platforms", "AutoPop Trophies", "Candlestick",
        ...
        "(cod|of duty|mw2|warzone)(?s).*(dmz|jack link|service|crossbow|code|items|camo|points|boost|hyper|unlock|mountain dew|warzone|skin|level|card|\\bXP\\b)",
