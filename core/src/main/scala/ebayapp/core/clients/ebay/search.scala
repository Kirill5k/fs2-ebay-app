  ...
  object EbaySearchParams {
    ...
    private object VideoGameSearchParams extends EbaySearchParams {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "villager(?s).*animal crossing", "animal crossing(?s).* (\\d+M|item|cafe|park|bell|mil|diy|material|ticket|recipe|fossil|dino|egg|gold)",
        ...
        "(cod|of duty|mw2|warzone)(?s).*(code|items|camo|points|boost|hyper|unlock|mountain dew|warzone|skin|level|card|\\bXP\\b)", "Destiny 2", "Destiny.*taken king", "heatsink", "Destiny\\s+the coll",
