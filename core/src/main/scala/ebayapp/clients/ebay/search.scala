  ...
  object EbaySearchParams {
    implicit val videoGameSearchParams = new EbaySearchParams[ItemDetails.Game] {
      ...
      private val LISTING_NAME_TRIGGER_WORDS = List(
        ...
        "(gta|grand theft)(?s).* (fast run|character|cars|boost|cash|money|online|mill)",
        ...
        "borderlands(?s).* (level|lvl|takedown|damage|Teething|dmg|mayhem|lvl|cash|x50|legendary|money|mod)",
