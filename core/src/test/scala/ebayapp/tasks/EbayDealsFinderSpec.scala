...
import ebayapp.services.{CexStockService, EbayDealsService, NotificationService, ResellableItemService, Services}
...
class EbayDealsFinderSpec extends CatsSpec {
  ...
  "A VideoGamesEbayDealsFinder" should {
    ...
    "retry forever periodically" in {
      val services = mocks
      when(services.ebayDeals.find(any[SearchQuery], any[FiniteDuration])(any[EbayItemMapper[ItemDetails.Game]], any[EbaySearchParams[ItemDetails.Game]]))
      ...
      result.unsafeToFuture().map { r =>
        verify(services.ebayDeals).find(any[SearchQuery], eqTo(20.minutes))(eqTo(mapper), eqTo(params))
        r must be (())
      }
    }
  }
